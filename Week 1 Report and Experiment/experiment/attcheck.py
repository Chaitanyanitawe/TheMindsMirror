...
expName = 'attcheck'  # from the Builder filename that created this script
...
def setupData(expInfo, dataDir=None):
    ...
    thisExp = data.ExperimentHandler(
        ...
        originPath='C:\\Users\\praka\\Psychopy\\attcheck.py',
...
def run(expInfo, thisExp, win, globalClock=None, thisSession=None):
    ...
    welcomeScreenComponents = [textWelcome]
    ...
    while continueRoutine and routineTimer.getTime() < 3.0:
        ...
        if textWelcome.status == STARTED:
            ...
            if tThisFlipGlobal > textWelcome.tStartRefresh + 3-frameTolerance:
    ...
    _key_resp_allKeys = []
    # keep track of which components have finished
    trialComponents = [textContinue, key_resp]
    ...
    while continueRoutine:
        ...
        # if textContinue is starting this frame...
        if textContinue.status == NOT_STARTED and tThisFlip >= 0.0-frameTolerance:
            # keep track of start time/frame for later
            ...
            # update status
            textContinue.status = STARTED
            textContinue.setAutoDraw(True)
        
        # if textContinue is active this frame...
        if textContinue.status == STARTED:
            # update params
            pass
    ...
    trials = data.TrialHandler(nReps=1.0, method='random', 
        ...
        trialList=data.importConditions('imag3.xlsx'),
    ...
    for thisTrial in trials:
        ...
        continueRoutine = True
        # update component parameters for each repeat
        thisExp.addData('image.started', globalClock.getTime(format='float'))
        image_2.setImage(path)
        ...
        # keep track of which components have finished
        imageComponents = [image_2, polygon, mouse, text_2]
        ...
        while continueRoutine:
            ...
            if image_2.status == STARTED:
                # is it time to stop? (based on global clock, using actual start)
                if tThisFlipGlobal > image_2.tStartRefresh + 6-frameTolerance:
                    # keep track of stop time/frame for later
            ...
            if polygon.status == NOT_STARTED and tThisFlip >= 7-frameTolerance:
                # keep track of start time/frame for later
        ...
        for thisComponent in imageComponents:
            if hasattr(thisComponent, "setAutoDraw"):
                thisComponent.setAutoDraw(False)
        thisExp.addData('image.stopped', globalClock.getTime(format='float'))
        ...
        gotValidClick = False  # until a click is received
        # keep track of which components have finished
        crossComponents = [polygon_2, mouse_2]
