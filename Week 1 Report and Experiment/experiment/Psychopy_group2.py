...
def setupData(expInfo, dataDir=None):
    ...
    thisExp = data.ExperimentHandler(
        ...
        originPath="C:\\Users\\umesh\\OneDrive\\Desktop\\MIND'S MIRROR\\untitled_lastrun.py",
...
def run(expInfo, thisExp, win, globalClock=None, thisSession=None):
    ...
    text_2 = visual.TextStim(win=win, name='text_2',
        text='1.Click on the displayed star to continue.\n2.Press the number displayed to continue.\n       press any key to continue:)',
        font='Arial',
        pos=(0, 0), height=0.05, wrapWidth=None, ori=0.0, 
        color='white', colorSpace='rgb', opacity=None, 
        languageStyle='LTR',
    ...
    # --- Initialize components for Routine "number_display" ---
    ...
    # --- Initialize components for Routine "star_display" ---
    ...
    # --- Initialize components for Routine "focus_center" ---
    ...
    trials = data.TrialHandler(nReps=1.0, method='random', 
        ...
        trialList=data.importConditions('psychopy-1.xlsx'),
    ...
    for thisTrial in trials:
        ...
        # --- Prepare to start Routine "number_display" ---
        continueRoutine = True
        # update component parameters for each repeat
        thisExp.addData('number_display.started', globalClock.getTime(format='float'))
        text.setText(number)
        key_resp.keys = []
        key_resp.rt = []
        _key_resp_allKeys = []
        # keep track of which components have finished
        number_displayComponents = [text, key_resp]
        for thisComponent in number_displayComponents:
            thisComponent.tStart = None
            thisComponent.tStop = None
            thisComponent.tStartRefresh = None
            thisComponent.tStopRefresh = None
        ...
        # --- Ending Routine "number_display" ---
        for thisComponent in number_displayComponents:
            if hasattr(thisComponent, "setAutoDraw"):
                thisComponent.setAutoDraw(False)
        thisExp.addData('number_display.stopped', globalClock.getTime(format='float'))
        # check responses
        ...
        # the Routine "number_display" was not non-slip safe, so reset the non-slip timer
        routineTimer.reset()
        
        # --- Prepare to start Routine "star_display" ---
        continueRoutine = True
        # update component parameters for each repeat
        thisExp.addData('star_display.started', globalClock.getTime(format='float'))
        ...
        # keep track of which components have finished
        star_displayComponents = [polygon, mouse]
        for thisComponent in star_displayComponents:
            thisComponent.tStart = None
            thisComponent.tStop = None
            thisComponent.tStartRefresh = None
            thisComponent.tStopRefresh = None
        ...
        # --- Ending Routine "star_display" ---
        for thisComponent in star_displayComponents:
            if hasattr(thisComponent, "setAutoDraw"):
                thisComponent.setAutoDraw(False)
        thisExp.addData('star_display.stopped', globalClock.getTime(format='float'))
        # store data for trials (TrialHandler)
        ...
        # the Routine "star_display" was not non-slip safe, so reset the non-slip timer
        routineTimer.reset()
        
        # --- Prepare to start Routine "focus_center" ---
        continueRoutine = True
        # update component parameters for each repeat
        thisExp.addData('focus_center.started', globalClock.getTime(format='float'))
        # keep track of which components have finished
        focus_centerComponents = [polygon_2]
        for thisComponent in focus_centerComponents:
            thisComponent.tStart = None
            thisComponent.tStop = None
            thisComponent.tStartRefresh = None
            thisComponent.tStopRefresh = None
        ...
        # --- Ending Routine "focus_center" ---
        for thisComponent in focus_centerComponents:
            if hasattr(thisComponent, "setAutoDraw"):
                thisComponent.setAutoDraw(False)
        thisExp.addData('focus_center.stopped', globalClock.getTime(format='float'))
        # using non-slip timing so subtract the expected duration of this Routine (unless ended on request)
        if routineForceEnded:
            routineTimer.reset()
        else:
